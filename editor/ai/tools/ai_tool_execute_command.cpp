...
bool _is_blocked_command(const String &p_command) {
	...
	// Block commands that could cause catastrophic damage outside the project.
...
// Check if command tries to operate on paths outside the project directory.
bool _has_dangerous_path(const String &p_command, const String &p_project_dir) {
	String cmd = p_command.strip_edges();

	// Block absolute paths outside the project dir for destructive commands.
	// Only check for rm, mv, cp with absolute paths.
	...
	// Scan for absolute paths that aren't under the project.
	// Simple heuristic: look for "/" followed by non-space at start of path tokens.
