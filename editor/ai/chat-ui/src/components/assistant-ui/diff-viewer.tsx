...
/**
 * Keeps only `contextLines` unchanged lines around each change,
 * replacing the rest with a "collapsed" placeholder.
 */
function filterContext(
  ...
  for (let i = 0; i < lines.length; i++) {
    if (keep[i]) {
      if (collapsed > 0) {
        items.push({ kind: "collapsed", count: collapsed });
        collapsed = 0;
      }
  ...
  if (collapsed > 0) items.push({ kind: "collapsed", count: collapsed });
  return items;
}
...
function CollapsedSeparator({ count }: { count: number }) {
  return (
    <div
      data-slot="diff-viewer-collapsed"
      className="flex items-center gap-2 px-4 py-0.5 text-xs text-muted-foreground select-none bg-muted/40"
    >
      <span className="flex-1 border-b border-dashed border-muted-foreground/30" />
      <span>{count} lines hidden</span>
      <span className="flex-1 border-b border-dashed border-muted-foreground/30" />
    </div>
  );
}
...
export type DiffViewerProps = Partial<SyntaxHighlighterProps> &
  VariantProps<typeof diffViewerVariants> & {
    ...
    showIcon?: boolean;
    showStats?: boolean;
    /** Only keep this many unchanged lines around each change. */
    contextLines?: number;
    /** Max visible lines before collapsing (0 = show all). */
  ...
  return (
    ...
    >
      {parsedFiles.map((file, fileIndex) => {
        const displayItems =
          contextLines != null
            ? filterContext(file.lines, contextLines)
            : file.lines.map((line): DisplayItem => ({ kind: "line", line }));

        const lineItems = displayItems.filter((d) => d.kind === "line");
        const totalVisibleLines = lineItems.length;
        const needsCollapse =
          maxCollapsedLines > 0 &&
          !expanded &&
          totalVisibleLines > maxCollapsedLines;
          ...
          <div key={fileIndex} data-slot="diff-viewer-file">
            ...
            <div data-slot="diff-viewer-content" className="overflow-x-auto">
              {displayItems.map((item, idx) => {
                if (item.kind === "collapsed") {
                  return <CollapsedSeparator key={`c-${idx}`} count={item.count} />;
                }
                lineCount++;
                if (needsCollapse && lineCount > maxCollapsedLines) return null;
                return (
                  <DiffViewerLine
                    key={idx}
                    line={item.line}
            ...
            {needsCollapse && (
              <button
                ...
                className="flex w-full items-center justify-center gap-1 border-t py-1.5 text-xs text-muted-foreground hover:bg-muted/60 transition-colors"
              >
                <ChevronDownIcon className="size-3" />
                Show {totalVisibleLines - maxCollapsedLines} more lines
              </button>
            )}
            {expanded && maxCollapsedLines > 0 && totalVisibleLines > maxCollapsedLines && (
              <button
                type="button"
